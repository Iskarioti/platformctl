...
# Stage tracked modifications and safe new files only.
git add -u

for path in .github .githooks config docs platform schema scripts shell templates vscode windows windows-terminal wsl \
            AGENTS.md CLAUDE.md KIMI.md README.md CHANGELOG.md VERSION bootstrap bootstrap.ps1 setup setup.ps1 workstation.json; do
  [[ -e "$path" ]] && git add -- "$path"
done

# Block obvious secret-bearing filenames.
if git diff --cached --name-only | grep -Eiq '(^|/)(\.env(\..*)?|id_rsa|id_ed25519|secrets?|credentials?)(/|$)|\.(pem|key|pfx|p12|kdbx)$'; then
...
# Rebase non-destructively if the remote branch advanced; never force push.
