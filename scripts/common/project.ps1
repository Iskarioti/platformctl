...
if ($env:OS -eq "Windows_NT") {
    ...
    if (-not (Get-Command wsl.exe -ErrorAction SilentlyContinue)) {
        throw "WSL is required for governed project commands on Windows."
    }
    ...
    $Dispatcher = "$RepoLinux/scripts/posix/project.sh"
    wsl.exe -d $Distro -- bash $Dispatcher $Action @Rest
    exit $LASTEXITCODE
}

& bash (Join-Path $Root "scripts/posix/project.sh") $Action @Rest
exit $LASTEXITCODE
