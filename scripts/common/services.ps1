...
if ($env:OS -eq "Windows_NT") {
    ...
    if (-not (Get-Command wsl.exe -ErrorAction SilentlyContinue)) {
        throw "WSL is required for development-service commands on Windows."
    }
    ...
    $Dispatcher = "$RepoLinux/scripts/posix/services.sh"
    wsl.exe -d $Distro -- bash $Dispatcher $Action @Rest
    exit $LASTEXITCODE
}

& bash (Join-Path $Root "scripts/posix/services.sh") $Action @Rest
exit $LASTEXITCODE
