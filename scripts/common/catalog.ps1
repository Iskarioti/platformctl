...
if ($env:OS -eq "Windows_NT") {
    ...
    if (-not (Get-Command wsl.exe -ErrorAction SilentlyContinue)) {
        throw "WSL is required for catalog commands on Windows - usage.jsonl is written to by WSL-side project/service commands."
    }
    ...
    $Dispatcher = "$RepoLinux/scripts/posix/catalog.sh"
    wsl.exe -d $Distro -- bash $Dispatcher $Action @Rest
    exit $LASTEXITCODE
}

& bash (Join-Path $Root "scripts/posix/catalog.sh") $Action @Rest
exit $LASTEXITCODE
