...
# See quality-docker.sh for the full rationale - identical eval, just run as
# a throwaway pod instead of a throwaway container.
...
try:
    ...
    # restarted (re-running this test in the same lab session) already has
    # the collection - tolerate that instead of failing on 409.
    if e.code != 409:
        raise
...
kubectl run rag-quality-probe -n "$NS" --rm -i --restart=Never \
  --image=python:3.13.15-slim-bookworm --command -- python3 -c "$PYCODE"
echo "PASS rag-pipeline kubernetes quality"
